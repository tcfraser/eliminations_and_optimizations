...
\usepackage{indentfirst}
\usepackage{pgfplots}
\pgfplotsset{compat=1.15} % https://tex.stackexchange.com/questions/81899/what-does-running-in-backwards-compatibility-mode-mean-and-what-should-i-fix-t
\usepackage{tikz}
...
\begin{equation}
    H = \left\{ x \in V \: \bigg| \: a^\intercal x = \sum_{i=1}^{n} a_i x_i \geq b \right\}
\end{equation}
...
\begin{center}
    ...
    \end{tikzpicture}
\end{center}
As previously mentioned, a polyhedron is the intersection of finitely many halfspaces and therefore corresponds to
\begin{equation}
    P = \left\{ x \in V \: \bigg| \: \bigwedge_{j=1}^{k} (a_{j}^\intercal x \geq b_j) \right\}
\end{equation}
...
\begin{equation}
    ...
    \end{tikzcd}
\end{equation}
In \Cref{eq:pullback_of_positive_orthant}, the morphism $j$ is simply the inclusion of the polyhedra $f^\ast ( \mathbb{R}^k_+ )$ into its ambient vector space $\mathbb{R}^n$ and the morphism $f'$ is provided by the restriction of $f$ onto $f^\ast ( \mathbb{R}^k_+ )$.
...
\tob{I think the following diagram should contain direct sums, not tensor products. If we work with linear instead of affine maps, then direct sums of vector spaces are biproducts, which implies that morphisms $X_1 \oplus \ldots \oplus X_n \longrightarrow Y_1 \oplus \ldots \oplus Y_m$ correspond to $m\times n$-\href{https://ncatlab.org/nlab/show/matrix+calculus}{matrices of morphisms}, to be composed by matrix multiplication. It's possible that a similar statement still holds with affine rather than linear maps, but I'm not sure}
...
\tob{Right. This is an application of the \href{https://ncatlab.org/nlab/show/pasting+law+for+pullbacks}{pullback lemma}}
...
\tob{The standard solution is to use \href{https://en.wikipedia.org/wiki/Grothendieck_universe}{Grothendieck universes}: using suitable set-theoretic assumptions, we can take proper classes to be sets as well, replacing ordinary sets by ``small sets'' and proper classes by ``large sets''. Then we can just use set-builder notation again, while noting that the resulting set may be large. \href{https://ncatlab.org/nlab/show/well-powered+category}{Well-poweredness} is a relevant keyword}
...
\tob{Right, I also feel that the standard notation isn't optimal. How about denoting objects by uppercase letters throughout, and then write
	\[
		\im(f) : \mathrm{Im}(f) \longrightarrow B \quad ?
	\]}
\ctodo{Clean up the discussion here, pullbacks are limits and images are some sort of colimits right?}
\tob{Yes, images are \emph{like} colimits in the sense of having a mapping-out universal property. I'm not sure whether they're \emph{actually} colimits}
...
\begin{proof}
    ...
    \begin{equation}
        ...
        \end{tikzcd}
    \end{equation}
    By Lemma~\ref{lem:mono_stable}, the argument is directly analogous to the diagram of \Cref{eq:right_adjoints_at_pullback}. The key difference is that while
    \begin{equation}
        \begin{tikzcd}[column sep=small]
